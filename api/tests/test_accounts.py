...
def test_create_account_success(test_context):
    ...
    res = make_post_request('/api/v1/accounts/', test_client, new_account, token)
...
def test_create_account_failure(test_context):
    ...
    res = make_post_request('/api/v1/accounts/', test_client, account_details, token)
...
def test_get_account_success(test_context):
    ...
    res = test_client.get(route, headers={ 'api-token': token })
...
def test_get_account_balance_during_transaction(test_context):
    ...
    with ThreadPoolExecutor(2) as pool:
        first = pool.submit(make_post_request, ('/api/v1/transactions/'), (test_client), (transaction_details), (token))

        get_path = '/api/v1/accounts/{}'.format(str(account.id))
        second = pool.submit(make_get_request, (get_path), (test_client), (token))
