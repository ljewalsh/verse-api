...
def create_transaction():
  ...
  if from_account is None:
      message = account_missing_message.format(from_account_id)
      return custom_response(message, 400)

  if to_account is None:
      message = account_missing_message.format(to_account_id)
      return custom_response(message, 400)

  if from_account.balance < amount:
      message = 'Account with id {} does not have enough money to make this transaction'.format(from_account_id)
      return custom_response(message, 400)
